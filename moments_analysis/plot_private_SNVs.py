...
def compute_and_save_likelihoods(passed_species):
    for species in passed_species.index:
        sfs_dat = moments_utils.load_sfs_counts(species)
        ...
        dem_model = moments_utils.prep_model_unscaled(moment_results.loc[species], moments.Demographics2D.split_mig, sample_sizes=sample_sizes)
        # prepare high migration model
        high_mig = moment_results.loc[species].copy()
        high_mig['m'] = 100
        ...
        sfs_dat.to_csv(config.intermediate_data_path / 'private_SNV_proba' / f'{species}_exclusive_snv_likelihoods.csv')


if __name__ == '__main__':
    moment_results = moments_utils.load_moments_results(config.project_path / 'moments_out' / '240714__split_mig__Hadza__Tsimane.csv')
    passed_species = pd.read_csv(config.intermediate_data_path / 'fig4_data.csv', index_col=0)

    # only need to run once
    # compute_and_save_likelihoods(passed_species)

    species_results = {}
    for species in passed_species.index:
        sfs_dat = pd.read_csv(config.intermediate_data_path / 'private_SNV_proba' / f'{species}_exclusive_snv_likelihoods.csv', index_col=0)
    ...
    for i, species in enumerate(passed_species.index):
        ...
        ax = axes_flat[i]
        ax.plot(obs_prob, label='Observed', marker='o', linestyle='', color=color_data, alpha=0.7, zorder=4)
    ...
    ax.plot(xs, obs_prob, label='Observed', marker='o', linestyle='', color=color_data, alpha=0.7, zorder=10)

    colors = [color_null, color_inferred]
    true_split = moment_results.loc[example_species_name, 'Tsplit']
    ...
    plt.savefig(config.fig_path / '20250212_private_snv_proba_inferred_example.pdf', dpi=300, bbox_inches='tight')