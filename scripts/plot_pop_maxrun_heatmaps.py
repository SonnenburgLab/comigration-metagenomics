...
all_assignments = []
fig_dir = Path('figs/240627_pop_maxrun.pdf')

threshold = 700
stat_func = lambda x: (x['max_run']>threshold).mean()
...
with PdfPages(fig_dir) as pdf:
    for species_name, species_res in full_res_annotated.groupby('species'):
        ...
        pop_comp_mat = pairwise_utils.compute_pop_comparisons(species_res, stat_func)
        ...
        g = sns.clustermap(pop_comp_mat.fillna(0), mask=pop_comp_mat.isna(), cmap='coolwarm', vmin=0, vmax=1, annot=True,
        ...
        g.ax_cbar.set_title(f'{species_name}\n\n\nFrac pairs with signal of recent transfer')
