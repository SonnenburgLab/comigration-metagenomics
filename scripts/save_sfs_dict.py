...
all_mag_df = []
if saving_full:
    output_path = config.sfs_path / f'{config.databatch}_full'
else:
    output_path = config.sfs_path / config.databatch
...
for species_name in pairwise_helper.metadata.get_species_list():
    ...
    if saving_full:
        species_snv_helper.save_dadi_data_dict(pops=pairwise_helper.metadata.get_all_pops(), allowed_mags=None, output_path=output_path)
    else:
