...
metadata = metadata_utils.MetadataHelper('240509')
all_assignments = []
fig_dir = Path('figs/240627_pop_divergence.pdf')

stat_func = lambda x: x['div'].mean()
...
with PdfPages(fig_dir) as pdf:
    for species_name, species_res in full_res_annotated.groupby('species'):
        ...
        g = sns.clustermap(pop_comp_mat.fillna(0), mask=pop_comp_mat.isna(), cmap='viridis', vmin=0, vmax=0.08, annot=True,
