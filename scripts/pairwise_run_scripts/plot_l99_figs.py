...
run_summary['implied_years_log'] = np.log10(run_summary['implied_years'])

allowed_comps = ['Hadza-Tsimane', 'China-HMP', 'China-MetaHIT', 'HMP-MetaHIT', 'Hadza-Hadza', 'Tsimane-Tsimane', 'China-China', 'MetaHIT-MetaHIT', 'HMP-HMP']
pop_comps = ['Hadza-Tsimane', 'China-USA', 'China-Germany', 'USA-Germany', 'Hadza', 'Tsimane', 'China', 'Germany', 'USA']
...
fig.savefig(config.fig_path / '241118_l99_plots' / 'violins_log_perc={}.pdf'.format(percid_threshold), bbox_inches='tight')
plt.close()
...
ax.set_xlim(xmin=0)
ax.set_xlabel('L99 years')
fig.savefig(config.fig_path / '241118_l99_plots' / 'violins_linear_perc={}.pdf'.format(percid_threshold), bbox_inches='tight')
...
fig.savefig(config.fig_path / '241118_l99_plots' / 'scatter_perc={}.pdf'.format(percid_threshold), bbox_inches='tight')
plt.close()
####### global scatter #######
fig, ax = plt.subplots(figsize=(3, 3), dpi=300)
pivot_summary = run_summary.pivot_table(index='species', columns='comp', values='implied_years_log')
sns.kdeplot(pivot_summary,  x='Hadza-Tsimane', y='China-HMP', fill=True, levels=levels)
sns.scatterplot(pivot_summary, x='Hadza-Tsimane', y='China-HMP', color='k', alpha=0.5, s=s)

row = pivot_summary.loc['Escherichia_coli']
plt.scatter(row['Hadza-Tsimane'], row['China-HMP'], color='r', s=10, label='E coli')
plt.legend()
...
plt.ylabel('China-HMP L99 years')
plt.savefig(config.fig_path / '241118_l99_plots' / 'scatter_global_perc={}.pdf'.format(percid_threshold), bbox_inches='tight')
plt.close()
